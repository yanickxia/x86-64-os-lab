...
先不要运行 `make check-c-kernel`。只根据下面的当前源码和规则，把答案写入[第 18 课学习记录](../notes/note-18.md)。这里没有要求猜测编译器的隐藏行为。
...
我们接下来不会继续扩写 bootloader，而会依次补上异常诊断、内存信息和分配器。当前单扇区 loader 最终会被更稳定的启动协议替代；今天学到的 ABI 边界仍然保留。
