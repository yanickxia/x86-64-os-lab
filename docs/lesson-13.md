...
**第一次阅读时先不要继续向下看，也不要运行命令。** 下面给出完成预测所需的全部输入；只依据这些材料和前 12 课形成的机器模型，把预测写入[学习记录](../notes/note-13.md)。不确定就写下直觉和置信度；错误答案不要删除，学完正文后再另写复盘。
...
第 4 题故意涉及尚未讲解的 NASM 编码选择。它**不能**从上述材料精确推出，请写猜测；这个错误或正确的直觉正是实验要记录的内容。第 7 题也不要求你手工解码 magic，只要求预测 CPU 会如何看待那些字节。
