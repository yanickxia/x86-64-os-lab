...
当前内存主线按状态边界推进：第 26 课建立 frame ownership，第 27 课建立 HHDM access 与 contents initialization，第 28 课建立 `#PF` 诊断，第 29 课只构造未激活的四级 mapping path。随后再补齐当前 kernel/stack/HHDM 所需映射并切换 CR3，把“数据结构正确”和“CPU 已安全使用”分开验证。
